...
class VAEmodel(BaseModel):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            if self.config['l_win'] == 24:
                ...
                conv_1 = tf.layers.conv2d(inputs=input_tensor,
                                          filters=self.config['num_hidden_units'] / 16,  # 卷积核个数
                                          kernel_size=(3, self.config['n_channel']),  # 卷积核大小
                                          strides=(4, 1),  # 步长
                                          padding='same',  # 填充方式
                                          activation=tf.nn.leaky_relu,
                                          kernel_initializer=init)
        ...
        with tf.variable_scope('decoder'):
            # tf.cond用于有条件的执行函数，当pred为True时，执行true_fn函数，否则执行false_fn函数
...
class lstmKerasModel:
    ...
    def plot_loss(self, history):
        ...
        savefig('../experiments/result/LSTM_loss.png')
    ...
    def plot_reconstructed_lt_seq(self, idx_test, config, model_vae, sess, data, lstm_embedding_test):
        ...
        for j in range(config['n_channel']):
            ...
            savefig(config['result_dir'] + "vae-lstm_long_seq_recons_{}.pdf".format(idx_test))
            fig.clf()
            plt.close()
    ...
    def plot_lstm_embedding_prediction(self, idx_test, config, model_vae, sess, data, lstm_embedding_test):
        ...
        fig.subplots_adjust(hspace=.4, wspace=.4)
        axs = axs.ravel()
        # 画出每个样本中11*6的图像 6原数据维度表示编码后的维度，画出每个维度embedding和原始的比较图
        for i in range(config['code_size']):
            # self.embedding_lstm_test表示的是经VAE处理后的编码数据，取后面k-1个样本进行比较
            axs[i].plot(np.arange(1, config['l_seq']), np.squeeze(self.embedding_lstm_test[idx_test, 1:, i]), "b")
            # lstm_embedding_test 表示的是经LSTM预测出来的后k-1个样本的结果，将两者进行对比
            axs[i].plot(np.arange(1, config['l_seq']), np.squeeze(lstm_embedding_test[idx_test, :, i]), "r-.")
            ...
            if i == config['code_size'] - 1:
                # axs[i].legend(('VAE\nembedding', 'LSTM\nembedding'))
                axs[i].legend(('LSTM_embedding', 'LSTM_embedding_predict'))
