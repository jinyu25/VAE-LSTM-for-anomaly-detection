...
class BaseModel:
    ...
    def define_loss(self):
        with tf.name_scope("loss"):
            ...
            self.weighted_reconstruction_error_dataset = weighted_reconstruction_error_dataset
            ...
            self.elbo_loss = self.weighted_reconstruction_error_dataset + self.KL_loss
...
class BaseTrain:
    ...
    def plot_train_and_val_loss(self):
        ...
        # self.train_loss表示在训练集上每一次迭代中每一个batch计算的损失，相当于计算了50*561（50次迭代，561个批量）的损失
        # plot(self.train_loss_ave_epoch, 'b-')
        plot(self.train_loss_ave_epoch, 'b--')
        # self.val_loss_ave_epoch表示一次迭代中所有batch在验证集上的平均损失，相当于迭代50次，计算了50个损失
        # self.iter_epochs_list列表的大小也是VAE模型迭代的次数
        # plot(self.iter_epochs_list, self.val_loss_ave_epoch, 'r-')
        plot(self.val_loss_ave_epoch, 'r')
        plt.legend(('training loss', 'validation loss'))
        plt.title('training loss over iterations (val @ epochs)')
        plt.ylabel('total loss')
        plt.xlabel('iterations')
        plt.grid(True)
        savefig('../experiments/result/VAE_loss.png')
